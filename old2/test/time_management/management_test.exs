defmodule TimeManagement.ManagementTest do
  use TimeManagement.DataCase

  alias TimeManagement.Management

  describe "users" do
    alias TimeManagement.Management.User

    import TimeManagement.ManagementFixtures
  ...
  describe "clocks" do
    alias TimeManagement.Management.Clock
    ...
    test "create_clock/1 with valid data creates a clock" do
      valid_attrs = %{status: true, time: ~U[2021-10-25 10:26:00Z]}

      assert {:ok, %Clock{} = clock} = Management.create_clock(valid_attrs)
      assert clock.status == true
      assert clock.time == ~U[2021-10-25 10:26:00Z]
    end
    ...
    test "update_clock/2 with valid data updates the clock" do
      clock = clock_fixture()
      update_attrs = %{status: false, time: ~U[2021-10-26 10:26:00Z]}

      assert {:ok, %Clock{} = clock} = Management.update_clock(clock, update_attrs)
      assert clock.status == false
      assert clock.time == ~U[2021-10-26 10:26:00Z]
    end
  ...
  describe "workingtimes" do
    ...
    test "create_workingtime/1 with valid data creates a workingtime" do
      valid_attrs = %{end: ~U[2021-10-25 10:27:00Z], start: ~U[2021-10-25 10:27:00Z]}

      assert {:ok, %Workingtime{} = workingtime} = Management.create_workingtime(valid_attrs)
      assert workingtime.end == ~U[2021-10-25 10:27:00Z]
      assert workingtime.start == ~U[2021-10-25 10:27:00Z]
    end
    ...
    test "update_workingtime/2 with valid data updates the workingtime" do
      workingtime = workingtime_fixture()
      update_attrs = %{end: ~U[2021-10-26 10:27:00Z], start: ~U[2021-10-26 10:27:00Z]}

      assert {:ok, %Workingtime{} = workingtime} = Management.update_workingtime(workingtime, update_attrs)
      assert workingtime.end == ~U[2021-10-26 10:27:00Z]
      assert workingtime.start == ~U[2021-10-26 10:27:00Z]
    end
